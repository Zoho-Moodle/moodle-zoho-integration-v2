...
// 2) قراءة صفحات الـ list
$all = [];
$page = $START_PAGE;
for ($p=0; $p<$MAX_PAGES; $p++) {
    $url = $baseurl . '?page=' . $page . '&per_page=' . $PER_PAGE . '&fields=' . urlencode(list_fields_query_param());
    $resp = curl_json($url, zoho_headers($token));
    $stats['zoho_requests']++;
    $stats['zoho_pages']++;
    $steps[] = ['step'=>'list_records','page'=>$page,'per_page'=>$PER_PAGE,'http_code'=>$resp['http_code'],'errno'=>$resp['errno'],'err'=>$resp['error'],'duration_sec'=>seconds($resp['duration']),'url'=>$url];
    log_step('🌐 Zoho list', ['page'=>$page,'per_page'=>$PER_PAGE,'http_code'=>$resp['http_code'],'errno'=>$resp['errno'],'duration_sec'=>seconds($resp['duration'])]);

    if ($resp['http_code'] == 204) break;
...
$stats['units_total'] = count($all);
log_step('✅ Total units received', ['count'=>$stats['units_total']]);
