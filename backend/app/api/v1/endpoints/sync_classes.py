...
from app.ingress.zoho.class_ingress import ingest_classes
...
class ClassSyncRequest(BaseModel):
    """Request body for class sync"""
    data: List[Dict[str, Any]]


class ClassSyncResponse(BaseModel):
    """Response for class sync"""
    status: str
    idempotency_key: str
    results: List[Dict[str, Any]]


@router.post("/classes", response_model=ClassSyncResponse)
def sync_classes(
    request: ClassSyncRequest,
    db: Session = Depends(get_db),
    x_tenant_id: Optional[str] = Header(None),
) -> ClassSyncResponse:
    """
    Sync classes from Zoho
    ...
    try:
        ...
        if cached:
            logger.info(f"Classes sync: Returning cached result (idempotency key: {req_hash})")
        ...
        # Ingest classes
        logger.info(f"Classes sync: Processing {len(request.data)} records (tenant: {tenant_id})")
        results = ingest_classes({"data": request.data}, db, tenant_id)
        ...
        logger.error(f"Classes sync error: {e}", exc_info=True)
